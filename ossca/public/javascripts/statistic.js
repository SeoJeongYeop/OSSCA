window.onload = function () {
  ...
  let containAbsence = true;
  let notOnlyMajor = true;
  ...
  fetchData();
  function fetchData() {
    const router = "/chartdata";
    let absence = "";
    let major = "";
    let slash = "/";
    if (!containAbsence) absence = "az";
    if (!notOnlyMajor) major = "pz";
    if (containAbsence && notOnlyMajor) slash = "";
    console.log(router + slash + absence + major);
    const promise = fetch(router + slash + absence + major)
      .then((response) => {
  ...
  function makePage(json) {
    ...
    function setOverallStat(json, nStudent = 1) {
      ...
      $("#commitPercent").text(
        (
          (json.totalCommit[annual - startAnnual] / nStudent / sumTotalCommit) *
          100
        ).toFixed(1) + "%"
      );
      $("#starNumerator").text(
    ...
    function setGraphData(annual, factor) {
      ...
      if (factor === "score") {
        ...
        sidData = json[`year${annual}`][`${factor}${scoreList[scoreIdx]}_sid`];
        deptData =
    ...
    function reloadChart(annual, factor) {
      ...
      if (chartFactor === "score") {
        ...
        deptData =
          json[`year${annual}`][`${factor}${scoreList[scoreIdx]}_dept`];
        sidStd =
          json[`year${annual}`][`${factor}${scoreCapitalList[scoreIdx]}SidStd`];
        deptStd =
          json[`year${annual}`][
